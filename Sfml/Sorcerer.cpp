...
#include <string>
    ...
    Sorcerer::Sorcerer(string Name, string gender)
    {
        setStrength(3);
        setConstitution(3);
        setWisdom(6);
	...
	Sorcerer::Sorcerer(int str, int Cst, int Wis, int Int, int Dex, int lck, int MP, int Cha, int Lvl)
	{
		setStrength(str);
        setConstitution(Cst);
        setWisdom(Wis);
	...
	void Sorcerer::notify(Sorcerer& NPC)
	{
		//ctl.Update(NPC);
	}

    //display user stats
    void Sorcerer::displayStats (Sorcerer& NPC)
    {
    ...
    //set extraPoints
    void Sorcerer::setExtraPoints (Sorcerer& NPC)
    {
        while(ExtraPoints != 0)
        {
            ...
            cout << "-We cannot change the the Level, class, next Level, charisma, HP and MP with extra points\n";
